...
function login($usuario, $senha, $destino){
	...
	if(mysql_num_rows($sel) == 0){//se não existir, mostra a mensagem abaixo
		...
		$idsession = date("H") . date("i") . date("s") . date("d") . date("m") . date("Y");
		$idsession = hash('sha512',$idsession);
...
function logoff($destino){
	...
	$sel = sel("sessoes","usuario = '$login' and senha = '$senha' and sessao = '$ids'","","");
	if(total($sel) != 0){
		$r = fetch($sel);
		...
		e("<meta http-equiv=\"refresh\" content=\"1;URL=$destino\">");
	}
}
...
function is_on(){
	...
	if(total($sel) == 0){
		...
		$ids = $_SESSION["idsession"];
		$sel2 = sel("sessoes","usuario = '$login' and senha = '$senha' and sessao = '$ids'","","");
		if(total($sel2) == 0){
			return false;
		}else{
			return true;
